/********
Developed by Pierluigi Dalla Rosa and Enrico Gueli
v0.1.5
*******/
...
window.tramontana = function () {
	...
	object.setupSocket = object.start = function (socketAddress, b) {
		...
		try {
			...
			object.socket.onmessage = function got_packet(msg) {
				...
				if (messageObject["m"] == "touched") {
				...
				else if (messageObject["m"] == "drag") {
					if (object.callbacks.drag != 0) {
						object.callbacks.drag(ipAddress, { "x": messageObject["x"], "y": messageObject["y"] });
					}
					else {
						jQuery.event.trigger('drag', { "address": ipAddress, "x": messageObject["x"], "y": messageObject["y"] });
					}
					try {
						drag(ipAddress, messageObject["x"], messageObject["y"]);
					}
					catch (e) {
					...
					try {
						attitude(ipAddress, messageObject["r"], messageObject["y"], messageObject["p"]);
					}
					catch (e) {
	...
	object.subscribeFunctions.touch = object.subscribeTouch = function (callback) {
		...
		if (callback != undefined) {
			object.callbacks.touched = callback;
		}
		else {
			object.callbacks.touched = 0;
		}
	}
	...
	object.subscribeFunctions.touchDrag = object.subscribeTouchDrag = function (callback) {
		object.subscribed.touchDrag = 1;

		object.sendMessage("{\"m\":\"registerTouchDrag\"}");
		if (callback != undefined) {
			object.callbacks.drag = callback;
			object.callbacks.touched = callback;
		}
		else {
			object.callbacks.drag = 0;
			object.callbacks.touched = 0;
		}
	}
	object.releaseTouchDrag = function () {
		object.subscribed.touchDrag = 0;
		object.sendMessage("{\"m\":\"releaseTouchDrag\"}");
	}
