...
window.BFtObject= function () {
		...
		object.setAllColorEmbedded = function(red,green,blue,intensity)
		{
			...
			{	
				...
				this.sendMessage("{\"m\":\"all\",\"r\":\""+Math.floor(red)+"\",\"g\":\""+Math.floor(green)+"\",\"b\":\""+Math.floor(blue)+"\",\"a\":\""+Math.floor(intensity)+"\"}");
			}
		}
		...
		object.takePicture = function(camera)
		{
			if(arguments.length>0 && isNumber(camera))
			{	
				this.sendMessage("{\"m\":\"takePicture\",\"c\":\""+camera+"\"}");
			}
			else
			{
