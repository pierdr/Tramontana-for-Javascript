...
window.BFtObject= function () {
        ...
        object.status 			   		= "INIT";
        object.subscribed 				= {distance:0,orientation:0,attitude:0,magnetometer:0,touch:0,audiojack:0,powersource:0,rxembedded:0,pir:0};
        object.callbacks 				= {videoended:0,socketopen:0,distance:0,orientation:0,attitude:0,magnetometer:0,touch:0,audiojack:0,powersource:0,rxembedded:0,pir:0};
        object.subscribeFunctions   	= {distance:0,orientation:0,attitude:0,magnetometer:0,touch:0,audiojack:0,powersource:0,rxembedded:0,pir:0};
        object.setupSocket = object.start = function(socketAddress,b){
			...
			try {
				...
				object.socket.onmessage =function got_packet(msg) {
					...
					else if(messageObject["m"] == "pir")
					{
						jQuery.event.trigger('pir', {"address":ipAddress});
						if(object.callbacks.pir!=0)
				        {
				        	object.callbacks.pir(ipAddress);
				        }
					}
	    ...
	    object.subscribeFunctions.pir=object.subscribePir = function(callback){
			var message="{\"m\":\"spir\"}";
			object.subscribed.pir =1;
			object.sendMessage(message);
			if(callback!=undefined)
	        {
	        	object.callbacks.pir=callback;
	    	}
	    	else
	    	{
	    		object.callbacks.pir=0;
	    	}
		}
		object.releasePir = function()
	    {
	    	object.subscribed.pir =0;
	      	object.sendMessage("{\"m\":\"dpir\"}");
	    }
